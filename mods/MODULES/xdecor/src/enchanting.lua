...
function enchanting:register_tools(mod, def)
	...
	for enchant in def.tools[tool].enchants:gmatch("[%w_]+") do
		...
		if original_toolcaps then
			...
			local item_name = mod .. ":enchanted_" .. tool .. "_" .. material .. "_" .. enchant
			local description = S("Enchanted @1 @2\n@3", arg1, arg2, arg3)
			...
			if minetest.get_modpath("mesecraft_toolranks") then
				minetest.override_item(item_name, {
					...
					description = mesecraft_toolranks.create_description(description, 0, 1),
...
if minetest.get_modpath("pigiron") then
	enchanting:register_tools("pigiron", {
		materials = "iron",
		material_desc = {iron = S("Iron")},
		tools = {
			axe    = {enchants = "durable, fast", desc = S("Axe")},
			pick   = {enchants = "durable, fast", desc = S("Pickaxe")},
			shovel = {enchants = "durable, fast", desc = S("Shovel")},
...
if minetest.get_modpath("magic_materials") then
	enchanting:register_tools("magic_materials", {
		materials = "arcanite",
		material_desc = {arcanite = S("Arcanite")},
		tools = {
			axe    = {enchants = "durable, fast", desc = S("Axe")},
			pick   = {enchants = "durable, fast", desc = S("Pickaxe")}, -- won't work as long as the item is named pickaxe_arcanite instead of pick_arcanite
			shovel = {enchants = "durable, fast", desc = S("Shovel")},
			sword  = {enchants = "sharp", desc = S("Sword")}
		},
	})
